...
class ControllerCatalogProduct extends Controller {
    ...
    public function importProductFromFile()
    {
        ...
        fclose($file_handle);
        $file_handle = fopen(DIR_UPLOAD . "content.csv", "r");
        $this->load->model('catalog/product');
        $i = 0;
        $j = 0;
        $k = 0;
        $stock_status_id = [7, 8, 5, 6];
        $quantity = [100, 200, 153, 147, 50, 533];
        $viewed = [1000, 2000, 15, 1470, 50, 533];
        $day = [20,10,30,5,2,15,18];
        $options = [];
        $shipping = 0;
        $sort_order = 0;
        while (!feof($file_handle)) {
            ...
            try {
                $pieces = explode(",", $line);
                $name = trim($pieces[4], '"');
                foreach ($category_find as $item => $value) {
                    foreach($value as $item2){
                        if(strpos($name,trim($item2)) !== false){
                            $category_counter = $item;
                            break 2;
                        }
                    }
                }
                $options['image'] = trim($pieces[0], '"');
                $options['sku'] = $pieces[1];
                $options['upc'] = $pieces[2];
                $options['price'] = $pieces[3];
                $options['name'] = $name;
                $options['viewed'] = $viewed[array_rand($viewed, 1)];
                $options['category_id'] = $category_counter;
                $options['quantity'] = $quantity[array_rand($quantity, 1)];
                $options['minimum'] = 1;
                $options['stock_status_id'] = $stock_status_id[array_rand($stock_status_id, 1)];
                $options['date_available'] =  date('y-m-d');
                $options['date_added'] =  date('y-m-d', strtotime('-'.$day[array_rand($day,1)].' days'));
                $options['shipping'] = $shipping;
                $options['status'] = 1;
                $options['sort_order'] = $sort_order;
                $options['description'] = trim($pieces[4], '"');
                $options['meta_description'] = trim($pieces[4], '"');
            } catch (Exception $e) {
                echo 'Caught exception: ', $e->getMessage(), "\n";
            }
            $this->model_catalog_product->addProductMy($options);
            $i++;
            $j++;
            if($i == 9000){
                break;
        ...
        $this->load->model('catalog/category');
        $this->load->model('catalog/product');
        $category_id = $this->model_catalog_category->getCategoryId();
        $product_id = $this->model_catalog_product->getProductId();
        foreach ($category_id->rows as $row) {
            try {
                $this->model_catalog_product->storeCategory($row['category_id']);
            } catch (Exception $e) {
                echo 'Caught exception: ', $e->getMessage(), "\n";
            }
        }
        foreach ($product_id->rows as $row) {
            try {
                $this->model_catalog_product->store($row['product_id']);
            } catch (Exception $e) {
                echo 'Caught exception: ', $e->getMessage(), "\n";
            }
        }*/
