		...
		//	echo '<pre>'.print_r( $children,1 ); die;
			...
			if ($category_info) {
				...
				if ($category_info['image']) {
					$this->mdata['thumb'] = $this->model_tool_image->resize($category_info['image'], $this->config->get($this->config->get('config_theme') . '_image_category_width'), $this->config->get($this->config->get('config_theme') . '_image_category_height'));
				} else {
					$this->mdata['thumb'] = '';
				}
				...
				$this->response->setOutput($this->load->view('pavblog/category', $this->mdata));
