			...
			foreach ($parts as $part) {
				...
				if ($query->num_rows) {
					...
					if ($url[0] == 'blogcategory_id') {
						$this->request->get['blogcategory_id'] = $url[1];
					}
					/** END OF PROCESSING TO DECORD REQUET SEO URL FOR PAVO BLOG MODULE **/

					if ($query->row['query'] && $url[0] != 'blogcategory_id' && $query->row['query'] && $url[0] != 'blog_id' && $query->row['query'] && $url[0] != 'information_id' && $url[0] != 'manufacturer_id' && $url[0] != 'category_id' && $url[0] != 'product_id') {
	...
	public function rewrite($link) {
		...
		foreach ($data as $key => $value) {
			if (isset($data['route'])) {
				...
				if ( ($data['route'] == 'pavblog/blogs' || $data['route'] == 'pavblog/blog' && $key == 'blog_id') ||
					 ($data['route'] == 'pavblog/blogs' || $data['route'] == 'pavblog/category' && $key == 'blogcategory_id') ||
